...
test("AI mapper converts dashboard plugin actions to proposed entries", () => {
  ...
  assert.equal(previews[0].entry?.metadata.note, "Lunch");
  assert.equal(previews[1].entry?.metadata.quality, "Good");
  assert.equal(previews[2].entry?.unit, "ml");
  assert.equal(previews[3].entry?.metadata.title, "Breathing");
});
...
test("AI mapper returns reasons for incomplete actions", () => {
  ...
  assert.match(previews[2].reason || "", /not mapped/);
});
